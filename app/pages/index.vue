...
export default {
  ...
  async asyncData () {
    ...
    const AuthStr = `Bearer ${process.env.airtableApiKey}`;
    const URL = `https://api.airtable.com/v0/${process.env.airtableDBId}/Config`;
    let response = await axios.get(URL, { headers: { Authorization: AuthStr } });
    let menuItems = response ? response.data.records.filter(item => !Vue._.isEmpty(item.fields) && item.fields.Live).map(item => {
      return {
        url: item.fields.URL || '',
    ...
    for (let index in subMenuGrouped) {
      ...
      for (let key in subMenuGrouped[index].children) {
        let childrenTemp = {
          name: key,
          children: subMenuGrouped[index].children[key].filter(item => item.url !== '').map(item => {
            return {
              name: item.actions,
              url: item.url
            };
    ...
    return { menuItems: itemMenuGrouped };
  }
}
</script>
